...
async function Main() {
  ...
  const configExist = fs.existsSync('./_CONFIGS/RONIN.json');
  if (!configExist) {
    logError(`Конфиг RONIN.json не существует. Копирую конфиг по умолчанию.`);
    logWarn(`Заполните RONIN.json`);
    fs.copyFileSync('./_CONFIGS/RONIN.json_', './_CONFIGS/RONIN.json');
    return false;
  };

  const CONFIG = require('./_CONFIGS/RONIN.json');
  // console.log(CONFIG);
  ...
  for await (let [i, row] of unready.entries()) {
    ...
    try {
      ...
      let balance = await getBalance(BOT, "RONIN");
      ...
      // BOT.tx_params["RONIN"].maxFeePerGas = 0;
      // BOT.tx_params["RONIN"].maxPriorityFeePerGas = 0;
      // Txn Type: 0 (Legacy) Rabby Wallet ???
      BOT.tx_params["RONIN"].type = CONFIG.TXN_TYPE;

      if (CONFIG.RANDOM_TXN_TYPE) {
        BOT.tx_params["RONIN"].type = shuffle([0, 2])[0];
      };
      ...
      for (let mintId of currentMints) {
        ...
        if (balance < CONFIG.MIN_BALANCE) {
          ...
          fs.appendFileSync(`./_LOGS/res.txt`, `${BOT.wallets["RONIN"].address}\tFALSE\t${balance}\n`, `utf-8`);
          delete BOT;
          continue;
        };
        ...
        if (tx === true) {
          ...
          if (tx === false) {
            ...
            logWarn(standardMsg + `| | Type: ${BOT.tx_params["RONIN"].type} | ${tx.hash}`);
            msg = consoleTime() + " | " + standardMsg + `| Транзакция в очереди | Type: ${BOT.tx_params["RONIN"].type} | ${CONFIG.EXPLORER}/${tx.hash}\n`;
            if (CONFIG.WAIT_TX) {
              await tx.wait();
        ...
        if (tx === true) {
          fs.appendFileSync(`./_LOGS/res.txt`, `${BOT.wallets["RONIN"].address}\tTRUE\t${balance}\n`, `utf-8`);
          delete BOT;
          continue;
        };
  ...
  async function createBot(BOT, privateKey, proxy) {
    ...
    BOT.providers["RONIN"] = await createProvider({
      RPC: CONFIG.RPC,
      proxy: BOT.proxy
    });
    ...
    // Создаем кошелек
    BOT.wallets = {};
    BOT.wallets["RONIN"] = await getWallet(privateKey, BOT.providers["RONIN"]);
    BOT.errors = {};
    BOT.errors["RONIN"] = 0;
